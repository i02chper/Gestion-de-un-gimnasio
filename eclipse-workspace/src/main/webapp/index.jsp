...
<%@ page import="business.user.UserInfo,data.dao.UserDAO,java.util.ArrayList,business.novedad.NovedadDTO" %>
	...
	<div class="resultados_container" style="top: 250px"><%
		if(novedades != null && !novedades.isEmpty()) {
			...
			for(int n_novedades = novedades.size() - 1; n_novedades >= 0; n_novedades--) {
				...
				<div class="novedad">
					<form method="post" id="novedad<%=cont%>" action="/modificarNovedad">
					...
					if(user != null && user.getTipo().equals("admin")) {%>
					<span class="control">
						<button type="submit" form="clase<%=cont%>">Modificar novedad</button>
					</span><%
					}%>
				</div><%
				cont++;
